...
const FR: &[(&str, &str)] = &[
    ...
    ("Press / to type a query, Enter to search the repositories and the AUR.",
     "Touche / pour saisir une requête, Entrée pour interroger les dépôts et l'AUR."),
    ("Searching for “{0}”…", "Recherche de « {0} »…"),
    ("Nothing matches “{0}”.", "Rien ne correspond à « {0} »."),
    ("Type at least two characters to search.",
     "Saisis au moins deux caractères pour chercher."),
    ("The AUR did not answer (network?).",
     "L'AUR n'a pas répondu (réseau ?)."),
    ("Unreadable AUR answer.", "Réponse de l'AUR illisible."),
    ("Search interrupted.", "Recherche interrompue."),
    ("out of date", "périmé"),
    ("Upstream", "Amont"),
    ("Votes", "Votes"),
    ("{0} · popularity {1}", "{0} · popularité {1}"),
    ("Maintainer", "Mainteneur"),
    ("none — orphaned", "aucun — orphelin"),
    ("⚑ Flagged out of date by a user.",
     "⚑ Signalé périmé par un utilisateur."),
    ("Built from a PKGBUILD, a script that runs under your account. paru will offer to show it before building.",
     "Compilé depuis un PKGBUILD, un script exécuté sous ton compte. paru proposera de l'afficher avant de compiler."),
    ("No package checked — space to check one",
     "Aucun paquet coché — espace pour en cocher un"),
    ("{0} asked for, {1} pulled in as dependencies.",
     "{0} demandé(s), {1} tiré(s) en dépendance."),
    ("AUR packages are built from source: their size and duration cannot be known beforehand.",
     "Les paquets AUR sont compilés depuis les sources : ni leur taille ni leur durée ne sont connues d'avance."),
    ("Install · {0} package(s)", "Installation · {0} paquet(s)"),
    ("query", "requête"),
    ("Check search results one by one — there is no “all”.",
     "Coche les résultats un par un — il n'y a pas de « tout »."),
    ("none", "rien"),
    ...
    ("↑↓ PgUp PgDn Home End scroll · every other key goes to paru",
     "↑↓ PgUp PgDn Début Fin défiler · les autres touches vont à paru"),
    ("Finished", "Terminé"),
    ("↑↓ PgUp PgDn Home End scroll · Enter to return",
     "↑↓ PgUp PgDn Début Fin défiler · Entrée revenir"),
    ("↑↓ PgUp PgDn Home End scroll · Enter back to the table",
     "↑↓ PgUp PgDn Début Fin défiler · Entrée revenir au tableau"),
    ("{0} packages · cache {1}", "{0} paquets · cache {1}"),
    ("System is up to date.", "Système à jour."),
    ("No orphans.", "Aucun orphelin."),
    ("No explicit package matches. The Search tab covers every package.",
     "Aucun paquet explicite ne correspond. L'onglet Recherche couvre tous les paquets."),
    ("{0} is installed, as a dependency. This tab lists only what nothing depends on.",
     "{0} est installé, comme dépendance. Cet onglet ne liste que ce dont rien ne dépend."),
    ("{0} explicit packages of {1} installed — dependencies are not listed here",
     "{0} paquets explicites sur {1} installés — les dépendances ne sont pas listées ici"),
    ...
    ("Needed only to build the above", "Nécessaire seulement pour compiler ce qui précède"),
    ("paru removes them again once the build is done.",
     "paru les retire une fois la compilation terminée."),
    ("paru will ask whether to remove them once the build is done.",
     "paru demandera s'il faut les retirer une fois la compilation terminée."),
    ("They stay installed afterwards — set RemoveMake in paru.conf to change that.",
     "Ils restent installés ensuite — RemoveMake dans paru.conf change ça."),
    ...
    ("Nothing to download: all cached", "Déjà en cache, rien à télécharger"),
    ("Sizes unknown before the build", "Tailles inconnues : compilation"),
    ("Summary", "Résumé"),
    ...
    ("No running process maps its files right now.",
     "Aucun processus n'en mappe les fichiers en ce moment."),
    ("{0} plugin: loaded on demand (dlopen) — the applications using it never declare it, alpm cannot see the link.",
     "Greffon {0} : chargé à la demande (dlopen) — les applications qui s'en servent ne le déclarent jamais, alpm ne peut pas voir le lien."),
    ("Debug symbols of {0}: only used to debug or profile it. Safe to remove otherwise.",
     "Symboles de débogage de {0} : ne servent qu'à le déboguer ou le profiler. Retirable sans risque sinon."),
    ("Python module: imported at run time by scripts and tools — a user of it leaves no alpm trace.",
     "Module Python : importé à l'exécution par des scripts et des outils — un utilisateur ne laisse aucune trace alpm."),
    ("Versioned compatibility library: kept for binaries linked against this old ABI. If nothing ever maps it, it is probably removable.",
     "Bibliothèque de compatibilité versionnée : gardée pour des binaires liés à cette ancienne ABI. Si rien ne la mappe jamais, elle est probablement retirable."),
    ...
    ("You did not ask for them; they arrive because something you asked for needs them. {0}{1}",
     "Tu ne les as pas demandés ; ils arrivent parce que ce que tu as demandé en a besoin. {0}{1}"),
